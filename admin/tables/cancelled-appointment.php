...
foreach ($result as $row) {
    ...
    $sub_array[] = date('F d, Y', strtotime($row['appointment_date']));
    $sub_array[] = $row['service'];
    $sub_array[] = $row['reason2'].' <br> '.$row['reason'];
    $sub_array[] = '
    <div class="d-flex flex-row gap-1">
        <button type="button" id="get_view" data-id="'.$row['appointment_id'].'" class="btn btn-primary"><i class="bi bi-eye-fill"></i></button>
    </div>
    ';
    $data[] = $sub_array;
}
